<nav class="sidebar sidebar-bunker" id="non-printable">
    ...
    @php
    // $user = Auth::user();
    $userAccess=\App\Models\UserRole::where('id',Auth::user()->userRole)->first();
    $routeAccessId = explode(',', $userAccess->userFeature ?? '');
    @endphp
        ...
        <nav class="sidebar-nav">
            <ul class="metismenu">
                ...
                <li class="{{ Route::is('userList') ? 'mm-active' : '' }}">
                    <a href="{{ route('userList') }}"><i class="typcn typcn-group-outline mr-2"></i>User List</a>
                </li>
                @endif
                @if (in_array(2, $routeAccessId))
                <li class="{{ Route::is('userRoleList') ? 'mm-active' : '' }}"><a href="{{ route('userRoleList') }}">
                        <i class="typcn typcn-th-menu-outline mr-2"></i>User Role List</a></li>
                @endif
                @if (in_array(3, $routeAccessId))
                <li class="{{ Route::is('userFeatureList') ? 'mm-active' : '' }}"><a
                        href="{{ route('userFeatureList') }}">
                        <i class="typcn typcn-lock-closed-outline mr-2"></i>User Features</a></li>
                @endif
            </ul>
        </nav>
    </div>
